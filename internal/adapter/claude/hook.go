// Package claude implements the Claude Code PreToolUse hook adapter. Its input
// and output JSON mirror the Codex adapter; only the registration name differs.
package claude
...
// Handle translates one Claude Code hook stdin JSON event to stdout. It emits
// additionalContext for PreToolUse+Bash with a non-empty command and is a silent
// no-op otherwise. It never denies, never asks, and never echoes the command or
// context onto stderr (fail-open, like the Codex adapter).
